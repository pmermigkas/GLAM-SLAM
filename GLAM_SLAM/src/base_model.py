...
class GaussianModelScaffold(BasicModel):
    ...
    def __init__(self, **model_kwargs):
        ...
        if self.use_multiple_mlp:
            print(f"{BOLD_GREEN_START}[GaussianMapper] Multiple MLPs: ENABLED with turn_threshold_detect={self.turn_threshold_detect}, turn_threshold_end={self.turn_threshold_end}, turn_min_keyframes={self.turn_min_keyframes}{RESET_FORMATTING}")
        else:
            print(f"{BOLD_GREEN_START}[GaussianMapper] Multiple MLPs: DISABLED{RESET_FORMATTING}")
        ...
        self.mlps_opacity.append(mlp_opacity)
        ...
        self.mlps_color.append(mlp_color)
    ...
    # @property
    def get_opacity_mlp(self, mlp_index):
        return self.mlps_opacity[mlp_index]   

    # @property
    def get_cov_mlp(self, mlp_index):
        return self.mlps_cov[mlp_index]
    
    # @property
    def get_color_mlp(self, mlp_index):
        return self.mlps_color[mlp_index]
    ...
    def extend_from_pcd(self, new_pcd : BasicPointCloud):
        new_points = torch.tensor(new_pcd.points).float().cuda()

        if self.voxel_size <= 0:
            init_dist = (knn(new_points, 4)[:, 1:] ** 2).mean(dim=-1).float().cuda()
            median_dist, _ = torch.kthvalue(init_dist, int(init_dist.shape[0]*0.5))
            self.voxel_size = median_dist.item()
            del init_dist
            torch.cuda.empty_cache()

        # Compute the integer coordinates of existing and new map points
        map_grid = torch.round(self.get_anchor / self.voxel_size).int()
        input_anchors_grid = torch.round(new_points / self.voxel_size).int()
 
        # Create a mask to find which voxels of input_anchors_grid fall into already-occupied voxels of map_grid
        matches = torch.all(input_anchors_grid.unsqueeze(1) == map_grid.unsqueeze(0), dim=2)
        # in_grid_mask will be a 1D boolean tensor of shape equal to the number of new points
        in_grid_mask = torch.any(matches, dim=1)

        # Filter out duplicates and keep only newly added points
        non_existent_grid = input_anchors_grid[~in_grid_mask]
        number_of_non_existent_points = non_existent_grid.shape[0]
        if number_of_non_existent_points > 0:
            # Get the unique rows
            new_anchors_grid_unique = torch.unique(non_existent_grid, dim=0) 
            new_anchors_unique = new_anchors_grid_unique * self.voxel_size

            number_of_unique_non_existent_points = new_anchors_unique.shape[0]
            new_offsets = torch.zeros((number_of_unique_non_existent_points, self.n_offsets, 3)).float().cuda()

            if number_of_unique_non_existent_points == 1:
                dist2 = torch.ones(1, device=new_anchors_unique.device, dtype=new_anchors_unique.dtype) * (self.voxel_size ** 2)
            else:
                # Handle 2-4+ points
                k_val = min(number_of_unique_non_existent_points, 4)
                dist2 = (knn(new_anchors_unique, k_val)[:, 1:] ** 2).mean(dim=-1)
        
            new_scaling = torch.log(torch.sqrt(dist2))[...,None].repeat(1, 6)
            
            new_rotation = torch.zeros([number_of_unique_non_existent_points, 4], device=new_anchors_unique.device).float()
            new_rotation[:,0] = 1.0
            new_feat = torch.zeros((number_of_unique_non_existent_points, self.feat_dim)).float().cuda()



            d = {
                "anchor": new_anchors_unique,
                "scaling": new_scaling,
            ...
            temp_anchor_demon = torch.cat([self.anchor_demon, torch.zeros([new_anchors_unique.shape[0], 1], device='cuda').float()], dim=0)
            del self.anchor_demon
            self.anchor_demon = temp_anchor_demon

            temp_opacity_accum = torch.cat([self.opacity_accum, torch.zeros([new_anchors_unique.shape[0], 1], device='cuda').float()], dim=0)
            ...
            # Update offset_denom
            # Add new slots for the new gaussians/offsets in demon tensor that counts how many times the gaussian was visible
            padding_offset_demon = torch.zeros([self.get_anchor.shape[0]*self.n_offsets - self.offset_denom.shape[0], 1],
                                           dtype=torch.int32, 
                                           device=self.offset_denom.device)
            self.offset_denom = torch.cat([self.offset_denom, padding_offset_demon], dim=0)

            # Prepare the gradient accumulator for the new gaussians
            padding_offset_gradient_accum = torch.zeros([self.get_anchor.shape[0]*self.n_offsets - self.offset_gradient_accum.shape[0], 1],
                                            dtype=torch.int32, 
                                            device=self.offset_gradient_accum.device)
            self.offset_gradient_accum = torch.cat([self.offset_gradient_accum, padding_offset_gradient_accum], dim=0)

    def add_mlps(self, training_args):
        mlp_opacity = nn.Sequential(
            nn.Linear(self.feat_dim+self.view_dim, self.feat_dim),
            nn.ReLU(True),
            nn.Linear(self.feat_dim, self.n_offsets),
        ...
        mlp_opacity = mlp_opacity.cuda()
        self.mlps_opacity.append(mlp_opacity)
        self.optimizer.add_param_group({
            'params': mlp_opacity.parameters(),
            'lr': training_args.mlp_opacity_lr_init,
            'name': f"mlp_opacity_{len(self.mlps_opacity)}"
        })
        ...
        mlp_cov = mlp_cov.cuda()
        self.mlps_cov.append(mlp_cov)
        self.optimizer.add_param_group({
            'params': mlp_cov.parameters(),
            'lr': training_args.mlp_cov_lr_init,
            'name': f"mlp_cov_{len(self.mlps_cov)}"
        })
        ...
        mlp_color = mlp_color.cuda()
        self.mlps_color.append(mlp_color)
        self.optimizer.add_param_group({
            'params': mlp_color.parameters(),
            'lr': training_args.mlp_color_lr_init,
            'name': f"mlp_color_{len(self.mlps_color)}"
        })
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            #The following are the initial MLPs that were added to the MLP lists during the model initialization
            {'params': self.mlps_opacity[0].parameters(), 'lr': training_args.mlp_opacity_lr_init, "name": "mlp_opacity_0"},
            {'params': self.mlps_cov[0].parameters(), 'lr': training_args.mlp_cov_lr_init, "name": "mlp_cov_0"},
            {'params': self.mlps_color[0].parameters(), 'lr': training_args.mlp_color_lr_init, "name": "mlp_color_0"},
        ]
        if self.appearance_dim > 0:
    ...
    def anchor_growing(self, grads, threshold, offset_mask, overlap):
        ...
        for i in range(self.update_depth):
            ...
            if candidate_anchor.shape[0] > 0:
                print(f"Added densify of size {candidate_anchor.shape[0]}")
    ...
    def generate_neural_gaussians(self, viewpoint_camera, visible_mask=None, ape_code=-1):
        ...
        if (self.use_multiple_mlp):
            new_mlp_checkpoints = self.turns
        else:
            new_mlp_checkpoints = [999999]

        # Determine the index of the MLP (0, 1, 2, etc ...)
        mlp_index = sum(viewpoint_camera.kfId > x for x in new_mlp_checkpoints)
    
        # get offset's opacity
        neural_opacity = self.get_opacity_mlp(mlp_index)(cat_local_view) # [N, k]
        ...
        # get offset's color
        if self.appearance_dim > 0:
            color = self.get_color_mlp(mlp_index)(torch.cat([cat_local_view, appearance], dim=1))
        else:
            color = self.get_color_mlp(mlp_index)(cat_local_view)
        color = color.reshape([anchor.shape[0]*self.n_offsets, 3])# [mask]

        # get offset's cov
        scale_rot = self.get_cov_mlp(mlp_index)(cat_local_view)
