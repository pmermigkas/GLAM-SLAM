...
def render(viewpoint_camera, pc, pipe, bg_color, iteration, render_mode, ape_code=-1):
    ...
    return_dict = {
        ...
        "visibility_filter" : torch.max(radii, dim=1).values > 0,
...
def prefilter_voxel(viewpoint_camera, pc, pipe, bg_color):
    ...
    # After upgrading to gsplat 1.5, the shape of the returned meta["radii"] has changed to (N, 2). In version 1.3, the second dimension was 1.
    # Authors now use a rectangle aabb (before it was a square) of the GS to compute which image tile it intersects with. This is a tighter aabb than before and results in less intersections thus make everything like 15% faster in average.
    # Since radii is now a tighter rectangle, you can recover it back to the original squares by taking the longer edge of the rectangle. That is, keep whichever element that is larger along the 2-dim.
    visible_mask[pc._anchor_mask] = torch.max(radii.squeeze(0), dim=1).values > 0
